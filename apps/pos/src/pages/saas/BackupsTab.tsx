            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                    <div className="space-y-4">
                        <div className="p-4 bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                            <div>
                                <h4 className="text-sm font-bold text-slate-800 dark:text-white">{t('backup.globalTitle')}</h4>
                            ...
                            <button 
                                ...
                                className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${isGlobalBackupLoading ? 'bg-slate-700 text-slate-500' : 'bg-blue-600 hover:bg-blue-500 text-slate-800 dark:text-white shadow-lg shadow-blue-600/20'}`}
                            >
                                {isGlobalBackupLoading ? t('backup.globalRunning') : t('backup.globalStart')}
                            </button>
                        </div>

                        <div className="p-4 bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800">
                            <h4 className="text-sm font-bold text-slate-800 dark:text-white">{t('backup.tenantTitle')}</h4>
                            <p className="text-[10px] text-slate-500 mt-1 mb-4">{t('backup.tenantDesc')}</p>
                            <div className="flex gap-2">
                                <select className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-white border border-slate-200 dark:border-slate-800 outline-none" id="tenant-backup-select">
                                ...
                                <button 
                                    ...
                                    className="bg-emerald-600 hover:bg-emerald-500 text-slate-800 dark:text-white px-4 py-2 rounded-xl text-xs font-black uppercase shadow-lg shadow-emerald-600/20 transition-all"
                                >
                                    {t('backup.pack')}
                                </button>
                            </div>
                     ...
                     <div className="space-y-4">
                        {[
                            ...
                            <div key={i} className="flex justify-between items-center p-3 bg-black/20 rounded-xl border border-white/[0.03]">
                                <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{p.label}</span>
            ...
            <SectionCard title={t('backup.archiveTitle')} icon={<FiDatabase className="text-emerald-400" />}>
                <div className="overflow-x-auto -mx-6">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] border-b border-slate-200 dark:border-slate-800">
                        ...
                        <tbody className="divide-y divide-white/[0.03]">
                            {backups.length > 0 ? backups.map(b => (
                                <tr key={b.id} className="hover:bg-blue-500/[0.03] transition-colors group">
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="font-bold text-sm text-slate-800 dark:text-white">{b.filename}</span>
                                                <span className="text-[9px] text-slate-500 font-mono">HASH: {String(b.id).slice(0, 16)}...</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5 text-sm font-medium text-slate-600 dark:text-slate-500 dark:text-slate-400">{(Number(b.size) / 1024 / 1024).toFixed(2)} MB</td>
                                    <td className="px-6 py-5">
                                    ...
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col">
                                            <span className="text-xs font-medium text-slate-600 dark:text-slate-500 dark:text-slate-400">{new Date(b.created_at).toLocaleDateString()}</span>
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button
                                                type="button"
                                                className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-white hover:bg-slate-100 dark:bg-slate-800 rounded-lg transition-all"
                                            ...
                                            <button 
                                                ...
                                                className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-black transition-all border ${
                                                    restoringId === b.id ? 'bg-amber-600 text-slate-800 dark:text-white border-amber-500' : 'bg-amber-600/10 text-amber-500 hover:bg-amber-600 hover:text-slate-800 dark:text-white border-amber-500/20'
                                                }`}
                                            >
